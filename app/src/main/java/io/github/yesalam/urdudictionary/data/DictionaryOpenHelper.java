...
public class DictionaryOpenHelper extends SQLiteAssetHelper {
    ...
    public Cursor getLimitedWord(String query,int limit){
        String sql = "select "+DictionaryEntry._ID+","+DictionaryEntry.COLUMN_WORD+" from "+DictionaryEntry.TABLE_NAME+" where "+DictionaryEntry.COLUMN_WORD+" like '"+query+"%' limit "+limit;
        Cursor result = getReadableDatabase().rawQuery(sql, null);
        return result ;
    }
